...
def common_parser():
    combined_ident = Combine(
        delimited_list(
            ansi_ident | mysql_backtick_ident | simple_ident,
            separator=".",
            combine=True,
        )
...
def parser(literal_string, ident, sqlserver=False):
    with Whitespace() as engine:
        ...
        case = (
            CASE
            + Group(
                ZeroOrMore((WHEN + expr("when") + THEN + expr("then")) / to_when_call)
            )("case")
        ...
        duration = (real_num | int_num | literal_string)(
            "params"
        ) + _standard_time_intervals
        ...
        select_column = (
            Group(expr("value") + alias | Literal("*")("value")).set_parser_name(
                "column"
            )
            / to_select_call
        )
        ...
        table_element = column_definition("columns") | table_constraint_definition(
            "constraint"
        )
